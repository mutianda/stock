...
<!--      <div v-for="(item,index) in shareList">{{ (item.code[0]==6? '1':'0')+item.code}} </div>-->
      <div class="card-view">
        <div class="card-item" v-for="(item,index) in shareList" @click="lookDetail(index)">
          <el-card class="card-box" :class="item.last.risePrecent>0?'card-red':'card-green'" @click="lookDetail(index)">
            <div slot="header" class="clearfix card-header">
              <span class="card-title">{{item.name}}  {{item.last.risePrecent}}%</span>
...
<div>{{item.code}}</div>
            ...
            <div  class="text item">
              量能： {{item.last.volumes}}
            </div>
            <div  class="text item">
              换手率：{{item.last.turnover}}%
            </div>
            <div  class="text item">
              成交额：{{item.last.moneyString}}
            </div>
          </el-card>
        </div>
...
<script>
    ...
    export default {
      ...
      computed:{
        shareList (){
          const arr = this.dblList.sort((a,b)=>b.last.risePrecent-a.last.risePrecent).map(item=>{
            item.last.moneyString = item.last.money/100000000>1?
              (Math.floor(item.last.money/100000000,2)+'亿'):(Math.floor(item.last.money/10000,2)+'万')
      ...
      // ["2021-02-09,54.66,53.11,55.40,52.12,90405,482797616.00,6.09,-1.45,-0.78,13.82"]
        ...
        getDbl(){
          ...
          this.dblList=[]
          console.log(this.dblList.map(item => item.name).length,this.dblList.map(item => item.name).join('、'));
